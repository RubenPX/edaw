export const consoleDecorations = {
    RoundedBorder: (color) => `border-left: 2px solid ${color}; border-right: 2px solid ${color}; padding: 0 4px; border-radius: 5px; `
};
...
export class ConsoleFormatter {
    ...
    static warn(badgeTitles, ...data) {
        if (badgeTitles instanceof Array)
            return ['%c' + badgeTitles.join(' | '), `${padding} ${badgeWarn}`, ...data];
        return ['%c' + badgeTitles, `${padding} ${badgeWarn}`, ...data];
    }
    static yellow(badgeTitles, ...data) {
        if (badgeTitles instanceof Array)
            return ['%c' + badgeTitles.join(' | '), `${padding} ${badgeYellow}`, ...data];
        return ['%c' + badgeTitles, `${padding} ${badgeYellow}`, ...data];
    }
    static succes(badgeTitles, ...data) {
        if (badgeTitles instanceof Array)
            return ['%c' + badgeTitles.join(' | '), `${padding} ${badgeSucc}`, ...data];
